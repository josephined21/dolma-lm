...
def format_question_ar(q, options):
    """format Arabic question and options"""
    lines = [f"سؤال: {q}", "خيارات:"]
    for i, opt in enumerate(options[:len(LETTER_CHOICES)]):
        lines.append(f"{LETTER_CHOICES[i]}. {opt}")
    lines.append("\nأجب بحرف الخيار الصحيح فقط (a-j)")
    return "\n".join(lines)
...
def apply_chat_template(user_content: str):
    ...
    if hasattr(tokenizer, "apply_chat_template"):
        messages = [
            {"role": "system", "content": "أنت مساعد فقط يجيب بحرف (a-j)"},
    ...
    for start_idx in range(0, len(split), batch_q):
        ...
        for ex in batch:
            ...
            if not q or len(options) < 2 or gold not in LETTER_CHOICES:
                continue
            user_prompt = format_question_ar(q, options)
