...
def format_question_fr(q, options):
    """format French question and options"""
    lines = [f"question: {q}", "options:"]
    for i, opt in enumerate(options[:len(LETTER_CHOICES)]):
        lines.append(f"{LETTER_CHOICES[i]}. {opt}")
    lines.append("\nréponds uniquement en indiquant la lettre (a-j) correspondant à la bonne réponse.")
    return "\n".join(lines)
...
def apply_chat_template(user_content: str):
    ...
    if hasattr(tokenizer, "apply_chat_template"):
        messages = [
            {"role": "system", "content": "tu es un assistent qui répond par une seule lettre a-j."},
    ...
    for start_idx in range(0, len(split), batch_q):
        ...
        for ex in batch:
            ...
            if not q or len(options) < 2 or gold not in LETTER_CHOICES:
                continue
            user_prompt = format_question_fr(q, options)
